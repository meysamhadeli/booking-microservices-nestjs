...
export class OtelDiagnosticsProvider {
  ...
  getTracer(): Tracer {
    ...
    return opentelemetry.trace.getTracer(configs.opentelemetry.serviceName!, configs.opentelemetry.serviceVersion ?? '1.0.0');
  }
  ...
  getMeter(): Meter {
    ...
    // This helps avoid trickier application load issues when other required dependencies are involved.
    return opentelemetry.metrics.getMeter(configs.opentelemetry.serviceName!, configs.opentelemetry.serviceVersion ?? '1.0.0');
  }
  ...
  getLogger(): Logger {
    ...
    return api_logs.getLogger(configs.opentelemetry.serviceName!, configs.opentelemetry.serviceVersion ?? '1.0.0');
  }
